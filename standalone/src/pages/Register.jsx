...
export default function Register() {
  ...
  function createToken() {
    ...
    const payload = btoa(JSON.stringify({ sub: '1', name, email, role: 'Admin', iat: Math.floor(Date.now() / 1000), exp: Math.floor(Date.now() / 1000) + 86400 }));
      ...
      <div style={{ background: 'white', padding: 40, borderRadius: 16, boxShadow: '0 4px 20px rgba(0,0,0,0.08)', width: '100%', maxWidth: 420 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <input type="password" value={confirm} onChange={e => setConfirm(e.target.value)} style={{ width: '100%', padding: '12px 14px', borderRadius: 8, border: '1px solid #d1d5db', fontSize: 14, outline: 'none' }} placeholder="••••••••" />
          </div>
