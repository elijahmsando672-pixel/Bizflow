  ...
  return (
    <div>
      <PageHeader title="Expenses" subtitle="Track and manage business expenses">
        ...
        <Btn color="var(--color-primary)" onClick={() => { setEditItem(null); setForm({ description: "", amount: "", category: "General", date: new Date().toISOString().split("T")[0] }); setModal(true); }}>
      ...
      {loading ? (
        ...
        <Table cols={["Description", "Category", "Amount", "Date", "Actions"]}
          rows={filtered.map((e) => [
            ...
            <span key="dt" className="text-muted-foreground">{e.date ? new Date(e.date).toLocaleDateString("en-GB") : "-"}</span>,
            <div key="ac" className="flex gap-1.5">
              <Btn small outline color="var(--color-accent-foreground)" onClick={() => { setEditItem(e); setForm({ description: e.description || "", amount: String(e.amount || ""), category: e.category || "General", date: e.date?.split("T")[0] || "" }); setModal(true); }}>
